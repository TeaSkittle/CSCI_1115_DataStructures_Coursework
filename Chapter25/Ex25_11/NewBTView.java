...
public class NewBTView extends Pane {

	private BST<Integer> tree = new BST<>();
	private double radius = 15; // Tree node radius
	private double vGap = 50; // Gap between two levels in a tree

	NewBTView(BST<Integer> tree) {
		this.tree = tree;
	...
	private void displayTree(BST.TreeNode<Integer> root, double x, double y, double hGap) {
		...
		if (root.right != null) {
			...
			//getChildren().add(new Line(x + hGap, y + vGap, x, y));
			getChildren().add(new Line(x + hGap, y / 2, x, y));
			// Draw the right subtree recursively
			//displayTree(root.right, x + hGap, y + vGap, hGap / 2);
			displayTree(root.right, x + hGap, y / 2, hGap / 2);
		}

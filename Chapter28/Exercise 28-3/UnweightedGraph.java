	...
	public SearchTree dfsNonRecurs(int v) {
		...
		while (!stack.isEmpty()) {
			...
			if (neighbors.get(x).size() == 0){
				...
				//
				// This section is complex, so I have a lot comments here
				// neighbors is list of lists, so has to be called as:
				// neighbors.get().get()to get actual value
				//
				// for ( int i = size of vertices in x's neighbor list and count towards 0 )
				for (int i = neighbors.get(x).size() - 1; i >= 0; i--) {
					Edge e = neighbors.get(x).get(i);	// grab Edge at index i, call e
					neighbors.get(x).remove(i);			// remove ending vertex from x's neighbor list
					if (!isVisited[e.v]) {				// if ending vertex of e is not visited

...
public class UnweightedGraph<V> implements Graph<V> {
  ...
  //    getACycle
  // ===============
  // pretty simple after following the psuedocode
  public List<Integer> getACycle() {
    // List of vertices
    List<Integer> list = new ArrayList<Integer>();
    for (int i = 0; i < vertices.size() - 1; i++){ list.add(i); }
    // boolean array to see if vertices has been visited
    boolean[] visit = new boolean[vertices.size()];
    // get parent of vertices
          ...
          if (neighbors.get(x).size() == 0){
            ...
            for (int i = 0; i < neighbors.get(x).size(); i++) {
              ...
              if (!visit[e.v]) {
                ...
                stack.push(e.v);
                visit[e.v] = true;
                // must call Integer here, else this error happens:

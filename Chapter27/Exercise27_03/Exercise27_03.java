...
public class Exercise27_03 {
  ...
  static class MyHashMap<K, V> implements MyMap<K, V> {
    ...
    //     Addded methods
    // ======================
    // Hash with key and a prime number
    public int hash2 ( int key ) {
      final int prime = 7;
      return prime - ( key % prime );
    }
    // change index variable for hashing
    public void changeIndex ( int index ) {
      index = ( index + hash2( index )) % capacity;
    }
    // compare index and key
    public boolean indexKey ( int index, K key ) {
    ...
    public V put( K key, V value ) {
      ...
      if ( get( key ) != null ) {
        while ( table[ index ] != null) {
          if ( indexKey( index, key )) {
            V oldValue = indexValue( index );
            table[ index ].value = value;
            return oldValue;
          } changeIndex( index );
        }
      } // Load factor stuff, same as Ex27_02
